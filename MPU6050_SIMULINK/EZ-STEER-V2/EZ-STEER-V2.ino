...
//  float accel_vector_length = sqrt(pow(accel_x,2) + pow(accel_y,2) + pow(accel_z,2));
 ...
 // Serial.print(F("#GYR:"));
  //Serial.println((int)unfiltered_gyro_angle_x);        //Gyroscope angle
 // Serial.print(F(","));
...
// Serial.print(F(","));
 ...
 Serial.write(map(angle_x,-90,90,-128,128));
if(Serial.available())
{
  

int8_t in = Serial.read();
int in_1 = map(round(in),-128,128,-255,255);
if(in != 0)
{
   if(in > 0)
   {
  //  motor.run(FORWARD);
 //   motor.setSpeed(abs(in_1));
   }
   if(in < 0)
   {
 //   motor.run(BACKWARD);
